...
export const CCForm = (props) => {
  ...
  const paymecheckout = async () => {
    ...
    let url =
      props.mode == 'staging'
        ? 'https://preprod.paymeservice.com'
        : 'https://ng.paymeservice.com'
    const response = await fetch(`${url}/api/pay-sale`, {
      method: 'post',
      headers: {
  ...
  const renderData = () => (
    <s.CardWrapper className='col-lg-5 col-md-6 col-sm-7 col-xs-7'>
      <span
        class='close'
        onClick={() => {
          setModal(null)
      ...
      <h3 style={{ alignContent: 'center' }} className='total-cost-h1'>
        Total Cost : {props.paymentInfo.amount}
      </h3>
  ...
  return (
    <>
      <div id='myModal' class='modal' style={{ display: modal }}>
        <div class='modal-content modal-content-background'>{renderData()}</div>
      </div>
    </>
  )
}
