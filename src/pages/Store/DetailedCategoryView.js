...
class DetailedCategoryView extends React.Component {
  ...
  componentWillMount() {
    let DetailedItem = this.props.products.filter(
  ...
  render() {
    ...
    let category_product = products.filter(
      (data) => data.category_id === this.props.match.params.category_id
    )
    let category = categories.filter(
      (data) => data.id === this.props.match.params.category_id
    )
            ...
            >
              <i
                className={`icofont-rounded-${
                  translation?.is_rlt === 'ar' ? 'right' : 'left'
                } back-page`}
              ></i>{' '}
              {translation?.back_to_menu || 'Back'}{' '}
            </a>
          </div>
        ...
        <div className='px-3 pb-3'>
          <div className='pt-0'>
            <h2 className='font-weight-bold text-white text-center'>
              {category[0].name[translation?.language_name]}
            </h2>
          </div>
          <div className='pt-2'></div>
        </div>
        <div
          className='category-bottom-v2-20'
          style={{
            backgroundColor: '#ffffff',
            borderTopLeftRadius: '35px',
            borderTopRightRadius: '35px',
            boxShadow: '0px -4px 12px -6px #9E9E9E',
          }}
        >
          <br />
          <div className='title d-flex align-items-center mb-3 mt-3 px-3'>
            <h6 className='m-0'>
              {' '}
              {translation?.menu_category_items || 'Items'}{' '}
            </h6>
          </div>
